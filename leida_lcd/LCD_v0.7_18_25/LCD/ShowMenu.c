...
void LanguageFunc(uint8_t key_value)
{
	...
	else if((key_value != 0) || (MenuManager.menu_status == 1))
	{
		...
		display_GB2312_string(3,32,"中文",0,0); 
		display_GB2312_string(5,32,"English",0,0); 
		MenuManager.menu_status = 0;
	}
}
...
void Show_Contents_Func(uint8_t key_value)
{
	...
	else if((key_value != 0) || (MenuManager.menu_status == 1))
	{
		...
		display_GB2312_string(1,8,((F_VAL *)(MenuManager.cur_menu->item_val))[0].val_name,0,0); 
		display_Signle_GB2312_string(3+(MenuManager.cur_menu->selected * 2),16+(8 * tmp_i),&tmp_ch,0,0);
		display_GB2312_string(3,32,"料高",0,0); 
		display_GB2312_string(5,32,"空高",0,0); 
		MenuManager.menu_status = 0;
	}
}
...
//				}
		...
		else if((key_value != 0) || (MenuManager.menu_status == 1))
		{
			...
			for(tmp_i  ; tmp_i < 3 * 2 ; tmp_i ++)
			{
				if(MenuManager.cur_menu->selected == tmp_i)
				{
					display_Signle_GB2312_string(7,16+(8 * (tmp_i - 3)),&((F_VAL *)(MenuManager.cur_menu->item_val))[1].char_val[tmp_i - 3],set_ok_flag,1);
				}
				else
				{
					display_Signle_GB2312_string(7,16+(8 * (tmp_i - 3)),&((F_VAL *)(MenuManager.cur_menu->item_val))[1].char_val[tmp_i - 3],0,0);
				}
			}
			MenuManager.menu_status = 0;
		}
