...
void display_graphic_32x32(uchar page,uchar column,uchar *dp)//显示16*16的点阵图像、汉字、生僻字或16*16点阵的其它图标
{
	uint i,j;
	for(j=0;j<4;j++)
	{
...
void display_128x64(uchar *dp) 	//显示128*64点阵图像
{
	uint i,j;
	for(j=0;j<8;j++)
	{
...
void display_graphic_5x8(uchar page,uchar column,uchar *dp) //5*8点阵图像
{
	uint i;
