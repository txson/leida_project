 ...
 * @LastEditTime: 2022-11-23 21:25:42
 * @LastEditors: herod
 * @Description: 
...
void sprint(float n,uint8_t *reChar,int zsize,int xsize,int flag)
//功能将浮点数n转成字符串，保存到以reChar地址开头的字符数组中（可实现插入）
...
{
//	volatile uint8_t tmp_val_i = 0;
//	volatile uint8_t tmp_val_j = 0;
//	memset(HighlowAdjustmentBuf,0,sizeof(HighlowAdjustmentBuf));
//	/*浮点型转字符串*/
//	//sprint(HighlowAdjustment,HighlowAdjustmentBuf,2,4,1);
//	sprintf((char*)HighlowAdjustmentBuf,"%.2lf",HighlowAdjustment);
//	HighlowAdjetMenu.subMenus[0]->menu_name = &HighlowAdjustmentBuf[0];
//	HighlowAdjetMenu.subMenus[1]->menu_name = HighlowAdjustmentBuf[1];
//	HighlowAdjetMenu.subMenus[2]->menu_name = HighlowAdjustmentBuf[2];
//	HighlowAdjetMenu.subMenus[3]->menu_name = HighlowAdjustmentBuf[3];
//	HighlowAdjetMenu.parent = &BasicSetMenu;
//	for(tmp_val_i = 0 ; tmp_val_i < 4;tmp_val_i++)
//	{
//		//display_GB2312_string(3,16*2,HighlowAdjustment,0,0);
//		//clear_screen();
//		//display_GB2312_string(4,16*2,HighlowAdjustmentBuf,0,0);
////		if('0' <= HighlowAdjustmentBuf[tmp_val_j]  && HighlowAdjustmentBuf[tmp_val_j]  <= '9')
////		{
//			HighlowAdjustmentSetMenu.subMenus[tmp_val_i]->menu_name = &HighlowAdjustmentBuf[tmp_val_i];
//			//tmp_val_i++;
////		}
//		
//	}
...
/**
 * [HighlowAdjFunc 高低位调整数据页面]
 * @author
 * @dateTime 2020-09-26
 * @param     key_value            	[当前按键的按键值]
 * @return                       	[无]
 */
