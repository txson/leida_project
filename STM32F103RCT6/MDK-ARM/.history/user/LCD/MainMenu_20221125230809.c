 ...
 * @LastEditTime: 2022-11-25 23:08:09
 * @LastEditors: herod
 * @Description: 主界面相关函数源文件
 * @FilePath: \MDK-ARM\user\LCD\MainMenu.c
 * XXXX
 */
#include "view_mgr.h"
#include "MainMenu.h"
 ...
 * @LastEditors: herod
 * @param {uchar} page			基纵坐标
 * @param {uchar} column		基横坐标
 * @param {uchar} *dp			需要传出的数据
 * @param {uint16_t} row_num	纵坐的显示长度
 * @param {uint16_t} col_num	横坐标的显示长度
 * @Description: lcd 显示
 */
...
void main_menu_func(uint8_t key_value)
{
	switch(key_value)
	{
		...
		default://按键A（确认按键）
		{
			...
			else if (!((cnt - cnt_start) % 10000))		/*1s刷新一次温度*/
			{
...
/**
 * @Date: 2022-11-23 23:14:30
 * @LastEditors: herod
 * @param {char*} data	需要显示的值
 * @param {int} len		需要显示的值的长度
 * @Description: 更新传感器的值
 */
...
void update_tempval(void)
{
	...
	ADC_Value = Get_Adc_Average(5);   //获取AD值
	Current_Temperature =( 1.43 - (float)ADC_Value*(3.3/4096) / 0.0043 ) + 25;	/*从寄存器读取温度*/
}

extern ADC_HandleTypeDef hadc1;
static __IO uint16_t Current_Temperature; 
/**
 * @Date: 2022-11-23 23:15:02
 * @LastEditors: herod
 * @Description: 更新温度显示
 */
