...
{
	int show_val = 0;				/*最终发送到lcd显示的数据*/
	volatile uint8_t current_x = 0;
	volatile int diff_pre_val = 0;	/*记录当前数据和上次数据的差值*/
...
// //	display_string_5x8(7,0,"0");
	...
	//if (!((cnt - cnt_start) %5000))		/*1s刷新一次温度*/
	{
		...
		/*	4.显示数值					*/
		/*显示数值*/
		diff_pre_val = (current_data * Y_POINT_MAX / (g_max_db / 0.9f))/*   -  ( previous_data * Y_POINT_MAX / Y_MAX_VAL)*/  ;
		
		for(current_x = 1;current_x <= 121 ; current_x++)
		{
			memcpy((void*)&previous_data,(void*)&data[(current_x - 1) * 2],2);		/*上一次的数据*/
			memcpy((void*)&current_data,(void*)&data[(current_x) * 2],2);			/*这一次的数据*/
			diff_pre_val = (current_data * Y_POINT_MAX / (g_max_db / 0.9f))   -  ( previous_data * Y_POINT_MAX / (g_max_db / 0.9f) ) ;
			do
			{
				show_val |= 1 << ((32 - (current_data * Y_POINT_MAX / (int)(g_max_db / 0.9f))  + diff_pre_val ));
				if(diff_pre_val > 0)
				{
					diff_pre_val--;
				}
				else if(diff_pre_val < 0)
				{
					diff_pre_val++;
				}
			}while(diff_pre_val != 0);
			point(4,8 + current_x,(uint8_t *)&show_val);
			show_val = 0;
		}
		/*	回波数值显示 字体大小 16*16	*/
