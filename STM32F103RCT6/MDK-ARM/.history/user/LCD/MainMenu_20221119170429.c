...
// //	display_string_5x8(7,0,"0");
	...
	//if (!((cnt - cnt_start) %5000))		/*1s刷新一次温度*/
	{
		...
		/*	4.显示数值					*/
		/*显示数值*/
		Difference = (current_data * Y_POINT_MAX / (g_max_db / 0.9f))/*   -  ( previous_data * Y_POINT_MAX / Y_MAX_VAL)*/  ;
		...
		for(tmp_i = 1;tmp_i <= 121 ; tmp_i++)
		{
			...
			Difference = (current_data * Y_POINT_MAX / (g_max_db / 0.9f))   -  ( previous_data * Y_POINT_MAX / (g_max_db / 0.9f) ) ;
			do
			{
				tmp_val |= 1 << ((32 - (current_data * Y_POINT_MAX / (int)(g_max_db / 0.9f))  + Difference ));
				if(Difference > 0)
				{
